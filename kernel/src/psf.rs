use byteorder::{ByteOrder, LittleEndian};

const PSF_MAGIC_IDENTIFIER: u32 = 0x864AB572;

pub struct PSFFont {
    magic: u32,
    pub version: u32,
    pub header_size: u32,
    pub flags: u32,
    pub num_glyph: u32,
    pub bytes_per_glyph: u32,
    pub height: u32,
    pub width: u32,
}

impl PSFFont {
    pub fn default() -> PSFFont {
        PSFFont {
            magic: PSF_MAGIC_IDENTIFIER,
            version: LittleEndian::read_u32(&PSF_FONT[4..8]),
            header_size: LittleEndian::read_u32(&PSF_FONT[8..12]),
            flags: LittleEndian::read_u32(&PSF_FONT[12..16]),
            num_glyph: LittleEndian::read_u32(&PSF_FONT[16..20]),
            bytes_per_glyph: LittleEndian::read_u32(&PSF_FONT[20..24]),
            height: LittleEndian::read_u32(&PSF_FONT[24..28]),
            width: LittleEndian::read_u32(&PSF_FONT[28..32]),
        }
    }

    pub fn read_glyph_row(&self, offset: usize, row: usize) -> u8 {
        let start_index = self.header_size as usize + (offset * self.bytes_per_glyph as usize);
        let row_index = start_index + row;

        if row_index >= PSF_FONT.len() {
            return 0x00;
        }

        PSF_FONT[row_index]
    }
}

// Temporarily defining entire font in code
pub static PSF_FONT: [u8; 4677] = [
    ,
];
